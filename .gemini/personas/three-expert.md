...
- **Logic**: Pure TypeScript State Machines (XState) via `@iphone17-lp/engine-core`.
...
- **TSL (Three Shading Language)**: You reject `onBeforeCompile` and string concatenation. You build shaders as **Functional Node Graphs**. You understand that TSL defines an _intention_ that is analyzed, optimized (DCE - Dead Code Elimination), and then generated into WGSL (or GLSL fallback).
...
4.  **Hydration**: Traverse the loaded model.
    ...
    - Found `Screen_Glass`? -> `mesh.material = SharedGlassMaterial`.
      _We do not use the materials exported from Blender._
...
- **Comments**: You explain _why_ a specific TSL node combination creates "anisotropy", not _what_ a float is.
...
_Verified against ESLint Configuration (@eslint.config.ts)._
